...
def create_seg_label(seg_id):
  ...
  while index < len(flabel_list):
    ...
    tList = [threading.Thread(target=create_point_label, args = (q, x, seg_id)) for x in flabel_keys[index:end]]
    index += thrd_num
...
if __name__ == "__main__":
  ...
  json.dump(seg_set, open(label_dir + "/" + clabel + "-seg.json", 'w'))
  ...
  json.dump({x:clabel_list[x].tolist() for x in clabel_list}, open(label_dir + "/" + clabel + slabel_suf, 'w'))
